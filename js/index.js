(function() {
  ...
  this.add_constraint = function() {
    var n;
    n = count_constraints();
    return $('div#group_constraints').append("      <div id = 'c" + n + "_constraint' class = 'constraint'>        <select name = 'c" + n + "_criteria'>          <option>Shortname</option>          <option>Fullname</option>          <option>Course Id #</option>          <option>Category</option>        </select>        <select name = 'c" + n + "_operator'>          <option>is</option>          <option>is not</option>          <option>contains</option>          <option>does not contain</option>        </select>        <span id = 'c" + n + "_search_term_0'>          <input name = 'c" + n + "_search_term_0' type = 'text'/>          <input id = 'c" + n + "_st_num' value = '1' type = 'hidden'/>          <img src = 'images/add.png' class = 'add_search_term'/>        </span>      </div>    ");
  };
  this.add_search_term = function(n) {
    var st_num;
    st_num = $("input#c" + n + "_st_num").val();
    $("div#c" + n + "_constraint").append("      <div id = 'c" + n + "_search_term_" + st_num + "' class = 'search_term'>        <div class = 'search_term_or'>OR</div>        <div class = 'search_term_input'>          <input name = 'c" + n + "_search_term_" + st_num + "' type = 'text'/>          <img src = 'images/delete.png' class = 'remove_search_term'/>        </div>      </div>    </div>    ");
